...
void th22mama(TH2* m, const char* filename, const char* comment="none")
{
    ...
    mama << "!FILE=Disk \n"
        ...
        "!TIME=" << tmp << "\n"
        "!CALIBRATION EkeV=6";
    const float cal[6] = { static_cast<float>(xax->GetBinLowEdge(1)), static_cast<float>(xax->GetBinWidth(1)), 0,
                           static_cast<float>(yax->GetBinLowEdge(1)), static_cast<float>(yax->GetBinWidth(1)), 0 };
    for(int i=0; i<6; ++i) {
