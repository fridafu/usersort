{
	...
	TH2F *h = new TH2F("h"," ",10,0.0,  10.548,10,4.267e-10,1.912e-07);
	ifstream strengthfile("strength.nrm");
	float strength[92],strengtherr[92],energyerr[92];
	float energy[911],trans[911];
	int i = 0;
   float a0 =  -0.7520;
   float a1 =   0.1200;
	float x;	
	while(strengthfile){
		...
		else{strengtherr[i-91] = x;}
		i++;
	}
	TGraphErrors *strengthexp = new TGraphErrors(92,energy,strength,energyerr,strengtherr);
    i = 0;
	...
	TLatex t;
	t.SetTextSize(0.05);
	t.DrawLatex(    2.010,5.736e-08,"^{xx}Yy");
	c1->Update();
