...
void plotstrength_theory_and_all_Y_data(){
    ...
    //ifstream strengthfiledone("strength_export.txt")
    ...
    double strength[n],strengtherr[n],energy[n],trans[n],transerr[n];
    ...
    double e_gdr[m], gdrdata[m],gdrdataerr[m],e_gdrfit[m];
...
//
    ...
    while(gdrfile){
        ...
        e_gdr[i] = x; // in MeV
        gdrdata[i] = y;
        gdrdataerr[i] = z; //  4.5% error corresponds to 1sigma according to Gry (email 19 Feb 2018)
        //cout << i << " " << x << endl;
        i++;
    }
    gdrfile.close();
...
//
    ...
    TGraphAsymmErrors *strengthexp = new TGraphAsymmErrors(64,energy,strength,energyerr,energyerr,strengtherr,strengtherr);
    //TGraph *strengthexp_D0up = new TGraph(n-1,energy,strength_D0up);
    //TGraph *strengthexp_D0down = new TGraph(n-1,energy,strength_D0down);
    //TGraph *strengthexp_Ggup = new TGraph(n-1,energy,strength_high);
    //TGraph *strengthexp_Ggdown = new TGraph(n-1,energy,strength_low);
    //TGraph *strengthexp_allup = new TGraph(n-1,energy,strength_allup);
    //TGraph *strengthexp_alldown = new TGraph(n-1,energy,strength_alldown);

    TGraphErrors *transexp = new TGraphErrors(n-1,energy,trans,energyerr,transerr); // The whole thing
    ...
    TGraphErrors *gdrstrength = new TGraphErrors(m-1,e_gdr,gdrdata,eg_err,gdrdataerr);
...
*/
    ...
    double par_array[7] = {E_r1,Gamma_r1,sigma_r1,E_r2,Gamma_r2,sigma_r2,temp};
    ...
    //fit_strength_E1->SetParLimits(0,14.,18.);
    //fit_strength_E1->SetParLimits(1,1.5,5.);
    //fit_strength_E1->SetParLimits(3,17.,19.);
    ...
    TF1 *fit_strength_all = new TF1("fit_strength_all",FitFunctionAll,2.3,16.5,12); // GDR, pygmy and upbend (12 parameters, but only 11 free parameters)
    ...
    fit_strength_all->FixParameter(7,9.1);//MeV
    fit_strength_all->FixParameter(8,2);//MeV  
    fit_strength_all->FixParameter(9,1.13);//mb/sr      
    ...
    fit_strength_all->SetParLimits(11,0.5,1);
    //fit_strength_all->SetParLimits(3,13.,15.);
    //fit_strength_all->SetParLimits(7,4.5,8.); 
    //fit_strength_all->SetParLimits(8,0.4,3); 
    //fit_strength_all->SetParLimits(9,0.4,10);     
    cout << endl;
    ...
    c1->Update();
    c1->Print("plotstrengthdata_theory_allZn68.pdf");
    c1->Print("plotstrengthdata_theory_allZn68.eps");
    c1->Print("plotstrengthdata_theory_allZn68.png");
    ...
    E1file = fopen("E1_gsf_68Zn_middle.txt","w");
    //E1file = fopen("E1_gsf_90Y_high.txt","w");
    fprintf(E1file," Z=  30 A=  68\n");
    ...
    M1file = fopen("M1_gsf_68Zn_middle.txt","w");
    //M1file = fopen("M1_gsf_90Y_high.txt","w");
    fprintf(M1file," Z=  30 A=  68\n");
